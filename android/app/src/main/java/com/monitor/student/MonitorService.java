...
public class MonitorService extends Service {
    ...
    public void onCreate() {
        ...
        Notification notification = new NotificationCompat.Builder(this, CHANNEL_ID)
                .setContentTitle("Classroom Monitor")
                .setContentText("Monitoring is active in background")
                .setSmallIcon(R.drawable.ic_launcher)
                .setPriority(NotificationCompat.PRIORITY_LOW)
                .build();

        startForeground(1001, notification);
        attachInvisibleOverlay();
    }
    ...
    // 1x1 Invisible view keeps window state active so OS doesn't kill camera
    private void attachInvisibleOverlay() {
        try {
            ...
            invisibleOverlay = new View(this);
            windowManager.addView(invisibleOverlay, params);
    ...
    private void initSocket(String url, String studentId) {
        try {
            ...
            socket.on("start-stream", args -> startBackgroundCamera());
            socket.on("stop-stream", args -> stopBackgroundCamera());
        } catch (Exception ignored) {}
    }
    ...
    public void onDestroy() {
        ...
        if (invisibleOverlay != null && windowManager != null) {
            try {
                windowManager.removeView(invisibleOverlay);
            } catch (Exception ignored) {}
        }
        if (socket != null) {
            socket.disconnect();
